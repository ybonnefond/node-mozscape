...
var Moz = (function(){
  ...
  Moz.prototype.exec = function(query, callback) {
    ...
    request(options, this._handleResponse.bind(this, callback));
  }

  Moz.prototype._handleResponse = function(callback, err, response, body) {
    if (err) callback(err);
  ...
  Moz.prototype.bitsToFlags = function(type, bits) {
    var result = [];
    for (var k in flags[type]) if (integer & flags[type][k]) result.push(k)
    return result;
  }
